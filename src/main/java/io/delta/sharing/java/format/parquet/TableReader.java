...
/**
 * A reader class for a delta sharing table.
 * It contains a queue of {@link ParquetReader} instances.
...
public class TableReader<T> {
    ...
    /**
     * Constructor.
     * @param paths A list of paths of parquet files that constitute a table.
     *              An individual instance of {@link ParquetReader} will be created for each file.
     ...
     * When the current reader in the queue is empty the queue will pop the reader and move to the next available file.
     * @return          An instance of template type if any is available in the available non-empty readers.
     *                  If the queue is empty it returns null.
     * @throws          IOException Transitive throws due to {@link ParquetReader#read()} call.
     * @implNote        The method is recursive. Recursion is used to abstract from rotation of readers in the queue.
     */
    ...
    /**
     * Reads N records from first non-empty {@link ParquetReader} instances.
     * If there are fewer than N records available it will return K available records.
     * The method is NOT idempotent.
     * When the current reader in the queue is empty the queue will pop the reader and move to the next available file.
     * @return          A list of N instances of template type if any is available in the available non-empty readers.
     *                  If the queue is empty before N records were fetched it will return the available records.
     *                  If called on an empty reader it will return an empty list.
     ...
     * @return The list containing the {@link ParquetReader} instances that will constitute the runtime queue.
     * @throws IOException Transitive due to a call to {@link LocalInputFile#LocalInputFile(Path)} constructor.
     */
